...
class ModelPusher:
    ...
    def export_module(self) -> ModelPusherArtifact:
        try:
            evaluatied_model_file_path = self.model_evaluation_artifact.evaluated_model_path
            exprot_dir = self.model_pusher_config.export_dir_path
            model_file_name = os.path.basename(evaluatied_model_file_path)
            ...
            shutil.copy(src= evaluatied_model_file_path, dst = exprot_model_file_path)
            logging.info(
                f"Trained model: {evaluatied_model_file_path} is copied in export dir: [{export_model_file_path}")
    ...
    def initiate_module_pusher(self) -> ModelPusherArtifact:
        try:
